...
class Trainer(BaseTrainer):
    ...
    def _log_predictions(
        ...
        results = list(zip(audio, probs, labels))
        shuffle(results)
        rows = {}
        for i, (audio_i, probs_i, label_i) in enumerate(results):
            if i >= examples_to_log:
                break
            rows[f"sample_{i}"] = {
                "audio": self.writer.wandb.Audio(audio_i.detach().cpu().numpy(), sample_rate=16000),
                "label": label_i.item(),
                "spoof_prob": probs_i[1].item(),
            }
